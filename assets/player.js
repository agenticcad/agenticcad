...
export function mountPlayer(root, tut, { onDone, next } = {}) {
  root.innerHTML = `<div class="pl">
    ...
    <div class="pl-app">
      <div class="pl-side">
        <div class="pl-hdr">${LOGO}<span>AgenticCAD</span><span class="ver">v0.9.1</span><span class="doc" data-doc>plate</span></div>
  ...
  const msgs = $('[data-msgs]'), chips = $('[data-chips]'), input = $('[data-input]'), cap = $('[data-cap]'), cmd = $('[data-cmd]'), panel = $('[data-panel]'), sk = $('[data-sketch]'), camBox = $('[data-cam]'), hud = $('[data-hud]'), tree = $('[data-tree]');
  ...
  const caption = html => { if (!html) { cap.classList.remove('show'); return; } cap.innerHTML = md(html); cap.classList.add('show'); };
  ...
  async function stepUser(s, q) {
    ...
    const m = document.createElement('div'); m.className = 'msg user'; m.innerHTML = (s.chips ? `<div class="chips">${s.chips.map(c => `<span class="chip">${esc(c)}</span>`).join('')}</div>` : '') + esc(s.text);
  ...
  async function stepAgent(s, q) {
    const m = document.createElement('div'); m.className = 'msg agent'; msgs.appendChild(m);
  ...
  async function stepTool(s, q) {
    const t = document.createElement('div'); t.className = 'tool closed';
    ...
    if (!q) await sleep(s.name === 'build_model' || s.name === 'build_cam' ? 1100 : 600);
  ...
  async function stepModel(s, q) {
    const mesh = tut.meshes[s.mesh]; hidePanel(); sk.classList.remove('show'); viewer.hideSketch(); ribbonOn(null); cmd.classList.remove('show'); viewer.hideMarker();
    ...
    setTree(mesh); caption(s.caption);
  }
  ...
  async function stepNote(s) { const m = document.createElement('div'); m.className = 'msg note'; m.innerHTML = '💡 ' + md(s.text); msgs.appendChild(m); scroll(); }
  async function stepCode(s) { showPanel('design.py', hlPython(s.text)); caption(s.caption); }
  async function stepGcode(s) { showPanel('plate.nc — head', hlGcode(tut.gcode_head || '')); caption(s.caption); }
  async function stepSketch(s, q) {
    ribbonOn('sketch'); hidePanel();
    ...
    sk.classList.add('show'); viewer.showSketch(s.plane, s.items, { reveal: q ? 0 : 1800 / state.speed }); state.sketchView = true; caption(s.caption);
  }
  ...
  async function stepToolpaths(s, q) {
    ...
    camBox.classList.add('show'); caption(s.caption);
  ...
  const IMPL = { user: stepUser, agent: stepAgent, tool: stepTool, model: stepModel, select: stepSelect, note: stepNote, code: stepCode, gcode: stepGcode, sketchmode: stepSketch, ribbon: stepRibbon, toolpaths: stepToolpaths };
  const dwell = s => Math.min(5200, 900 + ((s.caption || s.text || '').length) * 28);

  function reset() { cancel(); msgs.innerHTML = ''; chips.innerHTML = ''; input.innerHTML = ''; caption(null); cmd.classList.remove('show'); sk.classList.remove('show'); camBox.classList.remove('show'); hidePanel(); ribbonOn(null); viewer.clearToolpaths(); viewer.hideSketch(); viewer.hideMarker(); $('[data-tp]').classList.remove('on'); $('[data-end]').classList.remove('show'); state.fitted = false; }
  function ui() {
    ...
    const s = tut.steps[Math.max(0, state.idx - 1)]; $('[data-lab]').textContent = state.idx ? ({ user: 'you type', agent: 'agent replies', tool: 'tool: ' + (s.name || ''), model: 'model rebuilt', select: 'you click a face', note: 'tip', code: 'the script', gcode: 'G-code', sketchmode: 'sketch mode', ribbon: 'ribbon: ' + (s.tool || ''), toolpaths: 'toolpaths' })[s.kind] : 'ready';
    $('[data-play]').textContent = state.playing ? '❚❚ Pause' : (state.idx >= tut.steps.length ? '↻ Replay' : '▶ Play');
  }
  async function runStep(i, q) { const s = tut.steps[i]; if (!q) caption(null); await IMPL[s.kind](s, q); }
  async function play() {
  ...
  const first = tut.steps.find(s => s.kind === 'model'); if (first) { viewer.setModel(tut.meshes[first.mesh], { fit: true, fade: false }); setTree(tut.meshes[first.mesh]); }
  ui();
